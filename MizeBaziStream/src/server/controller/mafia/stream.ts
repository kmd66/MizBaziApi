...
import { MafiaControll, mafiaInstance } from './extensions';
...
export default class Stream extends Property {
    ...
    protected getUser(model: any): User | undefined {
        const room = mafiaDb().get(model.roomId);
    ...
    public async getRtpCapabilities(model: any) {
        ...
        SocketManager.sendToSocket('hubMafia', 'getRtpCapabilitiesReceive', user.connectionId, { rtpCapabilities });
    }
    ...
    public async consume(model: any) {
        ...
        SocketManager.sendToSocket('hubMafia', 'consumeReceive', user.connectionId, { params });
    }
